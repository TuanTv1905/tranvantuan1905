...
public:
	...
	void Introduce()          // gioi thieu cac phim dieu khien game
	{
		...
		textcolor(LIGHTGREEN);
		gotoXY(x, y);
		cout << "  A   : Dich sang trai";
		gotoXY(x, y + 2);
		cout << "  S   : Dich xuong";
		gotoXY(x, y + 4);
		cout << "  D   : Dich sang phai";
		gotoXY(x, y + 6);
		cout << "  W   : Xoay";
		gotoXY(x, y + 8);
		cout << "  R   : Choi lại";
			...
			do
			{
				if (_kbhit())
				{
					...
					case 'S':
						...
						break;
					case 'R':
						if (this->Bang_YesNo(Le_trai + 6, Le_tren + 5, " Choi lai ? "))    // goi bang YesNo de muon choi lai tu dau
						{
							Board.xoa();
							Board.DisplayBoard();
							delete currBlock;
					...
					case 27:  // Esc                                                          // goi bang quit game
						if (this->Bang_YesNo(Le_trai + 6, Le_tren + 5, "Thoat game ? "))
						{
							this->Xoa_bang_YesNo(Le_trai + 6, Le_tren + 5);
							clrscr();
							return res = 0;
				...
				gotoXY(1, 1);
				cout << End - Start;
			} while (float(End - Start) / CLOCKS_PER_SEC < 0.8);

			if (currBlock->Dich_xuong(Board) == false) //kiem tra cac khoi co the di chuyen xuong nua duoc khong
			{
	...
	int Bang_YesNo(int tung, int hoanh, string S)
	{
		...
		while (1)
		{
			...
			{
				c = _getch();
				if (c == 'a' || c == 'A')           // khi nhan phim enter
				 check = 1;
			}
			if (_kbhit())
			{
				c = _getch();
				if (c == 'd' || c == 'D')           // khi nhan phim enter
				 check = 0;
			}
			if (_kbhit())
			{
				c = _getch();
				if (c == 13)           // khi nhan phim enter
					return check;
			}
		}
	...
	void Xoa_bang_YesNo(int tung, int hoanh)        // xoa bang YesNo
	{
		for (int i = 0; i <= 6; i++)
		{
			for (int j = 0; j <= 24; j++)
			{
				gotoXY(tung + j, hoanh + i);
				cout << " ";
			}
		}
	}
